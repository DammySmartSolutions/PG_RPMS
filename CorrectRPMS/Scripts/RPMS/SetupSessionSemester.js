...
function LoadDataTable() {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/Rpms/LoadSessionSemester",
        success: function (data) {
            ...
            var datatableVariable = $('#table').DataTable({
                ...
                "columnDefs": [{ "width": "20%", "targets": 0 }, { "width": "5%", "targets": 1 }, { "width": "5%", "targets": 2 }],
                pageLength: 10,
                dom: "Bfrtip",
                buttons: [
            ...
            $('#table tbody').on('click', "#btnEdit", function () {
                ...
                id = Data["SessionSemesterID"]
                

                $("#ddlsession option").each(function () {
                    if ($(this).text() == Data["Session"]) {
                ...
                $("#ddlsemester option").each(function () {
                    if ($(this).text() == Data["Semester"]) {
                        $(this).attr("selected", "selected")
                    }
                });


                document.getElementById("UpdateValue").value = Data["SessionSemester"];


                Mainform.sessionsemesterid.value = id
            ...
            $('#table tbody').on('click', "#btnDelete", function () {
                ...
                var Data = table.row(currentrow).data();
                ID = Data["SessionSemesterID"]
                var facultyname = Data["SessionSemester"];



                var Result = confirm("You are about to delete " + facultyname + " Session_Semester")
                if (Result) {
...
function DeleteItem(ID) {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/Rpms/DeleteSessionsemester?id= " + ID + "",
        success: function (data) {
            alert("Session_Semester Deleted!");
            ID = 0;
            LoadDataTable();
