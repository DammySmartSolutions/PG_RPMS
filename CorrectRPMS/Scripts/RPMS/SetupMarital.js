    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#table tbody').on('click', "#btnEdit", function () {
                ...
                id = Data["MaritalID"]
                document.getElementById("txtmarital").value = Data["MaritalStatus"];



                Mainform.maritalid.value = id
            ...
            $('#table tbody').on('click', "#btnDelete", function () {
                ...
                var Data = table.row(currentrow).data();
                ID = Data["MaritalID"]
                var facultyname = Data["MaritalStatus"];



                var Result = confirm("You are about to delete marital status " + facultyname)
                if (Result) {
...
function DeleteItem(ID) {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/Rpms/DeleteMarital?id= " + ID + "",
        success: function (data) {
            alert("Marital Status Deleted!");
            ID = 0;
            LoadDataTable();
