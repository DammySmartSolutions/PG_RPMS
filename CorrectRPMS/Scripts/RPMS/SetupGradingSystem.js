...
function LoadDataTable() {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/Rpms/LoadGradingSystem",
        success: function (data) {
            ...
            var datatableVariable = $('#table').DataTable({
                ...
                "columnDefs": [{ "width": "20%", "targets": 0 }, { "width": "20%", "targets": 1 }, { "width": "5%", "targets": 2 }, { "width": "5%", "targets": 3 }],
                pageLength: 10,
                dom: "Bfrtip",
                buttons: [
...

                {
                    ...
                    'render': function (data, type, full, row) {
                        return '<a id=btnBrowse class="link-primary">Browse Grading</a>'
            ...
            $('#table tbody').on('click', "#btnBrowse", function () {
                ...
                id = Data["GradingSystemID"]
                

                window.location.href = "SetupGrading.aspx?id=" + id + "";

                Mainform.gradingsystemid.value = id
            ...
            $('#table tbody').on('click', "#btnEdit", function () {
                ...
                var Data = table.row(currentrow).data();
                id = Data["GradingSystemID"]
                document.getElementById("txtgradingsystem").value = Data["GradingSystem"];

                document.getElementById("UpdateValue").value = Data["GradingSystem"];
            ...
            $('#table tbody').on('click', "#btnDelete", function () {
                ...
                var Data = table.row(currentrow).data();
                ID = Data["GradingSystemID"]
                var facultyname = Data["GradingSystem"];



                var Result = confirm("You are about to delete grading system type " + facultyname)
                if (Result) {
...
function DeleteItem(ID) {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/Rpms/DeleteGradingSystemType?id= " + ID + "",
        success: function (data) {
            alert("Grading System Type Deleted!");
            ID = 0;
            LoadDataTable();
