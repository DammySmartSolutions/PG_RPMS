    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var datatableVariable = $('#table').DataTable({
                ...
                "columnDefs": [{ "width": "5%", "targets": 0 }, { "width": "10%", "targets": 1 }, { "width": "10%", "targets": 2 }, { "width": "5%", "targets": 3 }, { "width": "5%", "targets": 4 }],
                pageLength: 10,
                dom: "Bfrtip",
                buttons: [
                        ...
                        
                {
                    ...
                    'render': function (data, type, full, row) {
                        return '<a id=btnBrowse class="link-primary">Browse Local Govt.</a>'
            ...
            $('#table tbody').on('click', "#btnBrowse", function () {
                ...
                window.location.href = "SetupLocalGovt.aspx?id=" + id + "";

                Mainform.stateid.value = id
            ...
            $('#table tbody').on('click', "#btnEdit", function () {
                ...
                var Data = table.row(currentrow).data();
                id = Data["StateId"]
                document.getElementById("txtnationcode").value = Data["NationCode"];
                document.getElementById("txtstatename").value = Data["Statename"];
            ...
            $('#table tbody').on('click', "#btnDelete", function () {
                ...
                var Data = table.row(currentrow).data();
                ID = Data["StateId"]
                var facultyname = Data["Statename"];



                var Result = confirm("You are about to delete " + facultyname + " State")
                if (Result) {
...
function DeleteItem(ID) {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/Rpms/DeleteState?id= " + ID + "",
        success: function (data) {
            alert("Level Deleted!");
            ID = 0;
            LoadDataTable();
